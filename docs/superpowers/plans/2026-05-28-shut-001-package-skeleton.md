...
**Goal:** Create the initial Shuttle Swift package so `swift build` and `swift test` succeed with a minimal executable target and smoke test.

**Architecture:** This ticket only bootstraps the package and test harness. It adds a single executable target, a path dependency on `../PositronicKit`, and a minimal `ShuttleServerApp` type that the smoke test can import and validate. No HTTP, config, or runtime logic is added in this ticket.
...
Run: `swift test`
Expected: FAIL during compilation because `Package.swift` and the `ShuttleServer` target do not exist yet, or because `ShuttleServerApp` is undefined.
...
Run: `swift test`
Expected: FAIL during compilation because the `ShuttleServer` sources do not exist yet, or because `ShuttleServerApp` is undefined.
...
Run: `swift test`
Expected: PASS with `ShuttleServerSmokeTests.testStartupBannerMentionsTargetName` green.

- [ ] **Step 3: Run build to verify the executable product exists**

Run: `swift build`
Expected: BUILD SUCCEEDED with executable product `ShuttleServer`.

- [ ] **Step 4: Commit**

```bash
git add Package.swift Sources/ShuttleServer/ShuttleServerApp.swift Sources/ShuttleServer/main.swift Tests/ShuttleServerTests/ShuttleServerSmokeTests.swift docs/superpowers/plans/2026-05-28-shut-001-package-skeleton.md
...
- Type consistency: `ShuttleServerApp.makeStartupBanner()` is defined in the implementation task and referenced consistently in the test.
